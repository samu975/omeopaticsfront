...
const CrearBancoPreguntas = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Error al crear el banco de preguntas');
      toast.success('Banco de preguntas creado correctamente', {
        onClose: () => router.push('/banco-preguntas'),
        autoClose: 1200
      });
    } catch (err) {
      toast.error('Error al crear el banco de preguntas');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-base-200 p-8 flex flex-col items-center">
      <div className="w-full max-w-2xl card bg-base-100 shadow-xl p-8">
        <h1 className="text-2xl font-bold mb-6">Crear banco de preguntas</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {showQuestionForm && (
            <div className="card bg-base-200 p-4 space-y-4 mt-4">
              ...
              {error && <div className="alert alert-error mt-2 py-2 px-4 text-xs">{error}</div>}
            </div>
          )}

          {error && !showQuestionForm && <div className="alert alert-error mt-2 py-2 px-4 text-xs">{error}</div>}
          {success && <div className="alert alert-success mt-2 py-2 px-4 text-xs">{success}</div>}

          <div className="flex justify-end">
            <button type="submit" className={`btn btn-success ${loading ? 'loading' : ''}`}>Guardar banco de preguntas</button>
          </div>
        </form>
      </div>
...
export default CrearBancoPreguntas; 